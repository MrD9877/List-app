...
export type Item = { title: string; description: string; keyEntry: number; read: boolean };

export type AnimationList = { [key: number]: React.CSSProperties };

function getDirection(selected: number, moveTo: number) {
  if (selected < moveTo) {
    return "moveDown";
  } else {
    return "moveUp";
...
function getBlocks(selected: number, moveTo: number) {
  return Math.abs(selected - moveTo);
}
  ...
  return (
    <div className="flex flex-col gap-4 ">
      ...
      {typeof selected === "number" && typeof moveTo === "number" && (
        <button className="p-4 bg-black text-white rounded-2xl" onClick={() => moveBlock(selected, getDirection(selected, moveTo), getBlocks(selected, moveTo))}>
          Move
        </button>
      )}
    </div>
