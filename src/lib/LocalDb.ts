...
type SwapIndexType = { data: { index1: number; index2: number }; store: string };
...
export default class LocalDb {
  ...
  private swapOnSucess(event: Event, resolve: VoidFunction, reject: VoidFunction, obj: SwapIndexType) {
    const request = event.target as IDBRequest;
    const db = request.result;
  ...
  public async findOne(obj: FindOnsucessObj) {
    ...
    const getData: Promise<unknown> = new Promise((resolve, reject) => {
  ...
  public async putMany(objects: { store: string; data: unknown[] }) {
    this.onSucess = this.putOnsucess;
    const res = [];
  ...
  public async swapIndex(obj: SwapIndexType) {
    this.onSucess = this.swapOnSucess;
    const setData: Promise<OnSuccessResponse> = new Promise((resolve, reject) => {
      this.initialize(resolve, reject, obj);
    });
    const res = await setData;
